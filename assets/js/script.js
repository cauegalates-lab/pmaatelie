...
const storeProductCollections = [
  {
    id: 'mais-vendidos',
    title: 'Mais vendidos',
    subtitle: 'Os favoritos para presentes rápidos e especiais.',
  ...
  },
  {
    ...
    subtitle: 'Foto, nome ou frase em peças afetivas.',
  ...
  },
  {
    ...
    subtitle: 'Kits completos para presentear com carinho.',
  ...
  },
  {
    ...
    subtitle: 'Ideias para eventos, empresas e encomendas.',
...
function createStoreProductCard(product, index) {
  const category = getStoreCategory(product.categorySlug);
  const quantity = getStoreCartQuantity(product.id);
  const selectedClass = quantity > 0 ? ' is-in-cart' : '';
  const buttonClass = quantity > 0 ? 'store-product-add is-selected' : 'store-product-add';
  const quantityHidden = quantity > 0 ? '' : ' hidden';

  return `
    <article class="store-product-card${selectedClass}" style="--product-image-position: ${sanitizeStoreHtml(product.imagePosition || 'center')};" data-store-product-id="${sanitizeStoreHtml(product.id)}">
      <div class="store-product-card__image">
        ...
        <span class="store-product-card__qty" data-store-card-quantity="${sanitizeStoreHtml(product.id)}" aria-label="Quantidade selecionada"${quantityHidden}>${quantity}</span>
      </div>
      <div class="store-product-card__body">
        ...
        <div class="store-product-card__bottom">
          ...
          <button class="${buttonClass}" type="button" data-store-add-to-cart="${sanitizeStoreHtml(product.id)}" data-store-mobile-label="${quantity > 0 ? 'Adicionar +1' : 'Adicionar'}">Adicionar ao pedido</button>
        </div>
      </div>
    </article>
  `;
...
function addStoreProductToCart(productId, sourceButton = null) {
  ...
  persistStoreCart();
  renderStoreCart();
  syncStoreProductCardQuantities(productId);
  showStoreProductAddedFeedback(productId, sourceButton);
}

function getStoreCartQuantity(productId) {
  const item = storeState.cart.find((cartItem) => cartItem.id === productId);
  return item ? item.quantity : 0;
}

function syncStoreProductCardQuantities(productId = null) {
  storeQueryAll('[data-store-product-id]').forEach((card) => {
    const currentProductId = card.dataset.storeProductId;
    if (productId && currentProductId !== productId) return;

    const product = storeProducts.find((item) => item.id === currentProductId);
    const quantity = getStoreCartQuantity(currentProductId);
    const quantityBadge = card.querySelector('[data-store-card-quantity]');
    const addButton = card.querySelector('[data-store-add-to-cart]');

    card.classList.toggle('is-in-cart', quantity > 0);

    if (quantityBadge) {
      quantityBadge.textContent = quantity;
      quantityBadge.hidden = quantity <= 0;
    }

    if (addButton) {
      addButton.classList.toggle('is-selected', quantity > 0);
      addButton.dataset.storeMobileLabel = quantity > 0 ? 'Adicionar +1' : 'Adicionar';
      addButton.setAttribute(
        'aria-label',
        quantity > 0 && product
          ? `${quantity} ${quantity === 1 ? 'unidade selecionada' : 'unidades selecionadas'} de ${product.title}. Adicionar mais uma.`
          : `Adicionar ${product?.title || 'produto'} ao pedido`
      );
    }
  });
}

function showStoreProductAddedFeedback(productId, sourceButton = null) {
  const cards = storeQueryAll('[data-store-product-id]').filter((card) => card.dataset.storeProductId === productId);

  cards.forEach((card) => {
    card.classList.add('is-just-added');
    window.clearTimeout(card.storeAddedTimer);
    card.storeAddedTimer = window.setTimeout(() => card.classList.remove('is-just-added'), 900);
  });

  if (sourceButton) {
    sourceButton.classList.add('is-just-added');
    window.clearTimeout(sourceButton.storeAddedTimer);
    sourceButton.storeAddedTimer = window.setTimeout(() => sourceButton.classList.remove('is-just-added'), 900);
  }
}
...
function renderStoreCart() {
  ...
  syncStoreProductCardQuantities();
}
...
function persistStoreCart() {
  try {
    ...
    // O carrinho continua funcionando na sessão atual mesmo se o navegador bloquear armazenamento local.
  }
}
